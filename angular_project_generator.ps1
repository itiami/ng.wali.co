...
if ($LASTEXITCODE -eq 0) {
    ...
    # Wait for package.json to appear (indicating successful initialization)
    $maxRetries = 20
    $retryCount = 0
    while (-not (Test-Path "package.json") -and $retryCount -lt $maxRetries) {
    ...
    if (Test-Path "package.json") {
        ...
        # Run dev server
        Write-Host "Starting Angular development server on ${hostIP}:${portNumber}" -ForegroundColor Green
        ng serve --host $hostIP --port $portNumber --open --disable-host-check
    }
    else {
        Write-Host "ERROR: package.json not found after project creation!" -ForegroundColor Red
    }
}
else {
